...
- Position: 16 king zones × 2 relative colours × 6 piece types × 64 squares.
  It contains only piece identity, colour, square and king/input bucket.
- Attack: the existing X1 6-bit attacked/defended state, the existing X2
  occupied attacker-target edges, per-piece hanging/overload/pin/lower-value
  attacker flags, and king-zone pressure counts.
- Structure: pawn status masks (isolated, doubled, passed, candidate,
  connected, chain-supported, backward approximation, blocked), file state,
  pawn islands, centre pawn occupancy, king shelter, structural knight/bishop
  outposts, pawn colour complexes and passed-pawn blockers.
...
Material units are `P=1`, `N/B=3`, `R=5`, `Q=9`, `K=0`. Bucket version 1 uses
...
The bounds are quantiles measured from one million accepted records from
`train_3b_shuffled.data`, not twelve equal numeric ranges. Routing is hard to
one bucket. The model and format keep the bucket ID separate so interpolation
can be added later.
...
MagnusChess integration and runtime diagnostics are documented in
[MNUE_V2_ENGINE.md](MNUE_V2_ENGINE.md).

## Commands

```powershell
cd F:\MagnusChess\tools\mnue_v2_trainer
cargo test --offline
cargo run --release --features cuda --offline -- --dry-run --date 10000 --model-summary
cargo run --release --features cuda --offline -- --date 1000000
cargo run --release --features cuda --offline -- --date 0
```
