# MagnusChess MNUEv2 engine support
...
`export.rs`; this document only describes the engine integration.

## Architecture dispatch

MagnusChess keeps the existing legacy MNUE P2 loader and evaluator. The
`MNUEfile` option probes the first eight bytes:

- legacy files are passed to the existing P2 loader;
- `MNUEV2\0\0` files are passed to the strict format-v4 loader;
- a v2 file is accepted only when `format_version=4` and
  `architecture_id=7`.

Search dispatch remains `X1`, MNUEv2, legacy P2, NNUE, then HCE. Loading one
MNUE architecture unloads the other MNUE architecture, while the immutable
network storage remains process-global and accumulator stacks remain local to
each search worker.
...
It is not 1,536 wide. Compile-time assertions enforce every width.
...
avoiding the original repeated attacker-map reconstruction.
...
6. multiply by score scale 400 and round with `llround`.
...
`new_row - old_row`. Unmake applies the exact inverse integer deltas and then
...
state safely. Clear Hash does not own or mutate MNUEv2 state.

The accumulators are mathematically reversible i32 values. No saturating
integer operation is used.
...
`__builtin_cpu_supports("avx2")`; portable64 builds therefore retain a safe
scalar fallback.

## Material buckets

Bucket mapping version 1 uses material units `P=1`, `N/B=3`, `R=5`, `Q=9`
and inclusive upper bounds:
...
removals, cache activity, rebuilds, move-type distributions, p50/p90/p95/p99
row deltas, Attack accumulator ranges, clipping, slider-edge changes,
...
Normal search no longer reconstructs complete feature sets and no longer sorts
or diffs parent/child sparse vectors. The remaining cost is the real dirty
Attack/Structure semantic frontier, sparse row traffic for both perspectives,
and the selected 768-to-32 head. On the fixed 13-position one-thread benchmark,
the current native build is roughly 0.25M NPS versus the original 0.107M NPS.
The 0.8M performance gate has not been reached, so this implementation is not
yet performance-complete and Elo testing remains premature.
