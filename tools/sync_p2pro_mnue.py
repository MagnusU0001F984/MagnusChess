#!/usr/bin/env python3
"""
Convert a Bullet P2Pro quantised.bin into a MagnusChess .MNUE file.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="MagnusChess repository root. Default: this script's repository.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def sync_text_files(
    ...
    if set_embedded:
        ...
        replacements[root / "README.MD"] = [
            (
                r"embedded default MNUE P2(?:Pro)? network in the standard build",
                "embedded default MNUE P2Pro network in the standard build",
                "README project-scope embedded net text",
            ),
            (
                r"standard embedded P2(?:Pro)? network in the build",
                "standard embedded P2Pro network in the build",
                "README evaluation embedded net text",
            ),
            (
                r"the current default is (?:the legacy P2 net|a P2Pro net):",
                "the current default is a P2Pro net:",
                "README default-family text",
            ),
            (
